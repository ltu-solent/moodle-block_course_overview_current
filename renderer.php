...
/**
 * course_overview_current block rendrer
 *
 * @package    block_course_overview_current
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_course_overview_current_renderer extends plugin_renderer_base {

    /**
     * Construct contents of course_overview_current block
     *
     * @param array $courses list of courses in sorted order
     * @param array $overviews list of course overviews
     * @return string html to be displayed in course_overview_current block
     */
    public function course_overview_current($courses, $overviews) {
        $html = '';
        $config = get_config('block_course_overview_current');
        //if ($config->showcategories != BLOCKS_COURSE_OVERVIEW_CURRENT_SHOWCATEGORIES_NONE) {
            global $CFG;
        ...
        if ($this->page->user_is_editing() && (count($courses) > 1)) {
            $userediting = true;
            $this->page->requires->js_init_call('M.block_course_overview_current.add_handles');

            // Check if course is moving
            $ismovingcourse = optional_param('movecoursec', FALSE, PARAM_BOOL);
            $movingcourseidc = optional_param('courseid', 0, PARAM_INT);
        }

        // Render first movehere icon.
        if ($ismovingcourse) {
            // Remove movecoursec param from url.
            $this->page->ensure_param_not_in_url('movecoursec');

            // Show moving course notice, so user knows what is being moved.
            $html .= $this->output->box_start('notice');
            $ac = new stdClass();
            $ac->fullname = $courses[$movingcourseidc]->fullname;
            $ac->cancellink = html_writer::link($this->page->url, get_string('cancel'));
            $html .= get_string('movingcourse', 'block_course_overview_current', $ac);
            $html .= $this->output->box_end();

            $moveurl = new moodle_url('/blocks/course_overview_current/move.php',
                        array('sesskey' => sesskey(), 'moveto' => 0, 'courseid' => $movingcourseidc));
            // Create move icon, so it can be used.
            $movetofirsticon = html_writer::empty_tag('img',
                    array('src' => $this->output->pix_url('movehere'),
                        'alt' => get_string('movetofirst', 'block_course_overview_current', $courses[$movingcourseidc]->fullname),
        ...
        foreach ($courses as $key => $course) {
            ...
            if ($ismovingcourse && ($course->id == $movingcourseidc)) {
                continue;
            }
            $html .= $this->output->box_start('coursebox_current', "course-{$course->id}");
            $html .= html_writer::start_tag('div', array('class' => 'course_titlec'));
            // If user is editing, then add move icons.
            if ($userediting && !$ismovingcourse) {
                $moveicon = html_writer::empty_tag('img',
                        array('src' => $this->pix_url('t/move')->out(false),
                            'alt' => get_string('movecourse', 'block_course_overview_current', $course->fullname),
                            'title' => get_string('move')));
                $moveurl = new moodle_url($this->page->url, array('sesskey' => sesskey(), 'movecoursec' => 1, 'courseid' => $course->id));
            ...
            if (!empty($config->showchildren) && ($course->id > 0)) {
                // List children here.
                if ($children = block_course_overview_current_get_child_shortnames($course->id)) {
            ...
            if ($config->showcategories != BLOCKS_COURSE_OVERVIEW_CURRENT_SHOWCATEGORIES_NONE) {
                ...
                if ($currentcategory !== null) {
                    ...
                    if ($config->showcategories == BLOCKS_COURSE_OVERVIEW_CURRENT_SHOWCATEGORIES_FULL_PATH) {
            ...
            if ($ismovingcourse) {
                $moveurl = new moodle_url('/blocks/course_overview_current/move.php',
                            array('sesskey' => sesskey(), 'moveto' => $courseordernumber, 'courseid' => $movingcourseidc));
                $a = new stdClass();
                $a->movingcoursename = $courses[$movingcourseidc]->fullname;
                $a->currentcoursename = $course->fullname;
                $movehereicon = html_writer::empty_tag('img',
                        array('src' => $this->output->pix_url('movehere'),
                            'alt' => get_string('moveafterhere', 'block_course_overview_current', $a),
                            'title' => get_string('movehere')));
        ...
        return html_writer::tag('div', $html, array('class' => 'course_list_current'));
    }
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_course_overview_current', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_course_overview_current'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
        $url = new moodle_url('/my/index.php');
        $select = new single_select($url, 'mynumberc', $options, block_course_overview_current_get_max_user_courses(), array());
        $select->set_label(get_string('numtodisplay', 'block_course_overview_current'));
    ...
    public function hidden_courses($total) {
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            $output .= get_string('hiddencoursecount'.$plural, 'block_course_overview_current', $total);
        } else {
            $a = new stdClass();
            $a->coursecount = $total;
            $a->showalllink = html_writer::link(new moodle_url('/my/index.php', array('mynumberc' => block_course_overview_current::SHOW_ALL_COURSES_C)),
                    get_string('showallcourses'));
            $output .= get_string('hiddencoursecountwithshowall'.$plural, 'block_course_overview_current', $a);
        }
    ...
    public function welcome_area($msgcount) {
        ...
        $output .= $this->output->heading(get_string('welcome', 'block_course_overview_current', $USER->firstname));

        if (!empty($CFG->messaging)) {
            $plural = 's';
            if ($msgcount > 0) {
                $output .= get_string('youhavemessages', 'block_course_overview_current', $msgcount);
                if ($msgcount == 1) {
                    $plural = '';
                }
            } else {
                $output .= get_string('youhavenomessages', 'block_course_overview_current');
            }
            $output .= html_writer::link(new moodle_url('/message/index.php'),
                    get_string('message'.$plural, 'block_course_overview_current'));
        }
        $output .= $this->output->box_end();
